...
module dvi_monitor #(
    ...
    // A non-zero expected size turns capture-size mismatches into an immediate
    // test failure before a PPM header can describe storage outside MAX_*.
    parameter integer EXPECT_WIDTH = 0,
    parameter integer EXPECT_HEIGHT = 0,
    ...
    begin
        if (!init_done) begin
            ...
            if (meta_fd != 0) begin
                ...
                $display("[DVI_MON][WARN] Ensure %0s exists or pass +DVI_OUT_DIR.", `DVI_FRAME_OUTPUT_ROOT_REPO_REL);
            end
...
task dump_current_frame;
    ...
    begin
        ...
        if ((frame_width > 0) && (frame_height > 0)) begin
            if (((EXPECT_WIDTH > 0) && (frame_width != EXPECT_WIDTH)) ||
                ((EXPECT_HEIGHT > 0) && (frame_height != EXPECT_HEIGHT))) begin
                $fatal(1, "[DVI_MON] captured size %0dx%0d, expected %0dx%0d",
                       frame_width, frame_height, EXPECT_WIDTH, EXPECT_HEIGHT);
            end
            ...
            if (ppm_fd == 0) begin
                ...
                if (ppm_fd == 0) begin
                    ...
                    $display("[DVI_MON][WARN] Try +DVI_OUT_DIR=D:/.../%0s", `DVI_FRAME_OUTPUT_ROOT_REPO_REL);
                end
            end
            if (ppm_fd != 0) begin
                $fwrite(ppm_fd, "P3\n");
