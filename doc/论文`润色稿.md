<!--
稿件状态：可截稿版本（每个规模、每种方案连续测试 4 帧）
投稿前必须完成：作者与赛事信息、图5实拍图、图6-图7统计图，并使用原始日志逐项复核表6和正文数值。
统计边界：4 帧测试只报告原始值、均值、中位数和最大值，不报告 P99、置信区间或统计显著性。
-->
...
摘要：针对资源受限 FPGA SoC 中三维几何处理占用 CPU 时间、通用图形处理器资源代价较高的问题，提出一种帧级场景卸载架构。Scene Controller 在 FPGA 内完成模型加载、定点顶点变换、构面、背面剔除与 Painter 排序，SketchBook 复用二维光栅化与显示后端，使 CPU 仅需提交模型地址、变换参数和帧事务。系统采用 RGB332 双缓冲、64 bit 八像素写入和局部视口刷新，并在龙芯 FPGA 云平台与 RT-Thread Nano 上完成实现。在 S0～S4 规模下对 3 种软硬件划分方案分别连续测试 4 帧。结果表明，相较矩阵变换卸载，帧级卸载使 CPU 活跃周期中位数下降 97.27%～99.82%；实现占用 31.75% LUT、14.79% FF、23.29% BRAM 和 5.41% DSP，并满足时序约束。
...
实验按照顶点数 $V$ 和三角形数 $T$ 设置 S0～S4 五个规模点，见表5。每个规模点使用确定性资产和固定哈希，三种方案均从相同的动画状态开始，连续采集相同的 4 帧，以保证输入条件一致。
...
对同一资产、规模和轨迹帧，逐帧比较三角形命令序列与后页帧缓冲循环冗余校验（Cyclic Redundancy Check，CRC）。命令字段或帧缓冲 CRC 任一不一致时，将该帧标记为不等价，其性能值不进入结果统计。
...
延迟指标为从帧处理开始到 `PRESENT` 在显示边界完成的墙钟时间。30 frame/s 对应的单帧预算为 33.333 ms。每个规模、每种方案连续采集 4 帧，报告 4 个原始值及其均值、中位数和最大值。由于样本量较小，本文不计算 P99、置信区间或统计显著性；实时性仅依据 4 帧中的观测最大值是否超过 33.333 ms 进行描述。

支持指标包括 CPU 各软件阶段周期、Scene/AXI 事务量，以及查找表（Look-Up Table，LUT）、触发器（Flip-Flop，FF）、BRAM、数字信号处理单元（Digital Signal Processing，DSP）和最高工作频率。CPU 的 `active/wall` 只表示已插桩渲染代码在帧窗口中的占比，不等同于全系统 CPU 利用率或 RTOS 空闲率。

### 5.4 功能回归与四帧测试结果

功能回归覆盖 S0～S4 规模以及 `CPU_ONLY`、`CPU_MATMUL` 和 `SCENE_CONTROLLER` 三类路径，共归档 12 项测试；各测试均通过，方案间帧缓冲 CRC 一致。功能回归与性能采样分别执行，因此本文以归档回归结果作为输出等价性证据，不将性能批次中未重复记录的 CRC 数量写成等价帧数。

每个规模及方案连续采集 4 帧。帧级卸载在全部规模点上的 CPU 活跃周期中位数均低于其余两种方案，相对矩阵卸载的降幅为 97.27%～99.82%。S4 结果见表6，其中 `active/wall` 的变化方向与活跃周期一致。

**表6 S4 规模四帧测试结果（每种方案 n=4）**  
**Table 6 Four-frame results at the S4 scale (n=4 per scheme)**

| 指标 | `CPU_ONLY` | `CPU_MATMUL` | `SCENE_CONTROLLER` |
|---|---:|---:|---:|
| 活跃周期中位数 / cycle | 1,968,009 | 1,863,108 | 3,405 |
| `active/wall` 均值 / % | 82.64 | 82.25 | 0.32 |
| 观测最大帧延迟 / ms | 75.823 | 73.680 | 34.723 |

四帧测试中，帧级卸载在 S0～S2 的观测最大帧延迟不超过 20.663 ms，S3 和 S4 分别为 34.741 ms 和 34.723 ms，后两者略高于 33.333 ms 帧预算。这表明 CPU 活跃周期大幅下降并不等价于端到端帧延迟必然满足实时约束，较高负载下的几何处理、排序或光栅化链路仍可能成为瓶颈。上述结果只反映本次 4 帧序列，不用于推断长时间运行中的尾部延迟分布。

<!--
图6待生成：横轴 S0-S4，纵轴 CPU active cycles（对数坐标），三条折线分别为 CPU_ONLY、CPU_MATMUL、SCENE_CONTROLLER；折线点使用4帧中位数，旁边以浅色散点显示4个原始值，不绘制置信区间。
图7待生成：横轴 S0-S4，纵轴观测最大帧延迟/ms；三条方案曲线，并增加 y=33.333 ms 的红色虚线。
-->

**图6 不同规模下三级方案的 CPU 活跃周期中位数（待生成）**  
**Fig. 6 Median CPU active cycles of the three schemes across workload scales (figure pending)**

**图7 不同规模下三级方案的观测最大帧延迟（待生成）**  
**Fig. 7 Observed maximum frame latency of the three schemes across workload scales (figure pending)**
...
功能回归表明，三级软硬件划分方案在当前归档测试中输出一致。在每个规模、每种方案连续采集 4 帧的条件下，帧级卸载相对矩阵卸载使 CPU 活跃周期中位数降低 97.27%～99.82%；S0～S2 的观测最大帧延迟不超过 20.663 ms，S3 和 S4 分别为 34.741 ms 和 34.723 ms。冻结实现占用 31.75% LUT、14.79% FF、23.29% BRAM 和 5.41% DSP，并满足既定时序约束。结果说明，所提架构能够显著减少 CPU 渲染路径开销，但较高负载下仍受几何处理、排序或光栅化吞吐限制。本文不对 4 帧以外的延迟分布作统计外推，结论限于正交投影、无纹理、RGB332 且不超过 128 顶点和 192 三角形的受控低模场景。
...
| 表5 | 模型规模点 | 三线表 | 单栏 | 已完成 |
| 图6 | CPU 活跃周期随规模变化 | 折线图+原始散点 | 对数纵轴；每点为4帧中位数，叠加4个原始值 | 待生成 |
| 图7 | 观测最大帧延迟随规模变化 | 折线图 | 标出 33.333 ms 预算线 | 待生成 |
...
- 所有坐标轴写明量和单位；本稿样本量为4，不使用置信区间或误差线，优先叠加4个原始数据点。

### 仍需补齐的硬信息

- 作者顺序、指导教师、通信作者、电子邮箱、杯赛名称、报名编号 CICC1002311 和所获奖项。
- 使用原始日志复核 S0～S4、三级方案的4帧原始值，以及正文中的中位数、最大值和下降比例。
- 图5的原始显示画面，以及基于4帧真实数据生成的图6、图7。
