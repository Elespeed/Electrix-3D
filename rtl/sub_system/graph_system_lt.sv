...
// ----------------------------------------------------------------------------
// graph_system_lt : 3D Lite small-system wrapper (Phase 0x0).
// Copied from graph_system.sv; the ONLY change is the GRU instance uses
// gru_top_lt (the Lite shell) instead of gru_top.  All ports, parameters,
// wires, the GDU / DDR arbiter / DDR controller instances and the hierarchy
// instance names are identical so this is a drop-in for graph_system and stays
// compatible with gru_graph_system_tb_harness.svh hierarchical references.
// ----------------------------------------------------------------------------
module graph_system_lt #(
    ...
    // LITE DIFF: gru_top -> gru_top_lt (Lite 3D shell).  Port list unchanged.
    gru_top_lt u_gru_top (
