// ----------------------------------------------------------------------------
// gru_top_lt : 3D Lite top-level GRU shell (Phase 0x0).
//
// This file is a copy of the stable gru_top.sv with the Full-3D datapath
// REMOVED and replaced by the Lite path.  Differences vs gru_top.sv are limited
// to the Full-3D replacement (see "LITE DIFF" comments); all 2D engines, the
// register file, command / ext / blit / span FIFOs, the span arbiter, the AXI
// writer, the blit engine, the Fence logic, the command buffer / fetch path and
// every top-level port are intentionally identical so the Lite instance is a
// drop-in for gru_top inside graph_system_lt / soc_top_lv3lt_board.
//
// Lite has no Full-3D engines:
//   * gru_triangle_engine  -> gru_lite_triangle_engine (seq-gated drain shell)
//   * gru_depth_rw         -> removed (depth opcodes rejected at dispatch)
//   * texture / depth AXI arbitration branches removed (no such masters)
//
// Unsupported 3D opcodes uniformly: command consumed + GRU_STATUS_CFG_ERROR set
// + Fence retires.  Triangles are dispatched into the triangle FIFO and drained
// by the Lite shell (done + cfg_error).  CLEAR_DEPTH / TRIANGLE_Z are rejected
// at dispatch (cfg_error, no seq).  See docs/3dlite/phase0x0_隔离Lite路径开发.md.
// ----------------------------------------------------------------------------
`include "../../config.h"
`include "gru_defs.vh"
...
module gru_top_lt (
    ...
    logic depth_busy;
    // LITE DIFF: Lite triangle shell outputs (drain + retire + cfg_error).
    logic        lite_tri_cmd_done_pulse;
    logic        lite_tri_cfg_error_pulse;
    ...
    // LITE DIFF: Lite always recognizes triangle opcodes (no ENABLE_3D gate).
    assign cmd_head_is_textured_triangle = (cmd_head_opcode == `GRU_OP_TRIANGLE_TEXTURED) ||
                                           (cmd_head_opcode == `GRU_OP_TRIANGLE_TEXTURED_PC);
    assign cmd_head_is_triangle = (cmd_head_opcode == `GRU_OP_TRIANGLE_FLAT) ||
                                  (cmd_head_opcode == `GRU_OP_TRIANGLE_GOURAUD) ||
                                  cmd_head_is_textured_triangle;
    // LITE DIFF: depth opcodes are still detected (for deterministic rejection
    // below) but Lite has no depth/Z engine.
    assign cmd_head_is_depth = (cmd_head_opcode == `GRU_OP_CLEAR_DEPTH) ||
    ...
    // LITE DIFF: cmd_head_is_depth forces CFG_ERROR unconditionally (was
    // cmd_head_is_depth & ~depth_cfg_valid).  CLEAR_DEPTH / TRIANGLE_Z are
    // consumed via cmd_drop_fire, set CFG_ERROR, allocate no seq, and the Fence
    // retires naturally.  Triangles that reach the FIFO are drained by the Lite
    // shell, which also flags CFG_ERROR (not implemented yet).
    ...
    // LITE DIFF: no depth/Z engine, so never dispatch depth commands.  They are
    // rejected above (cmd_cfg_error -> cmd_drop_fire) and never reach the FIFO.
    assign depth_q_wr_en = 1'b0;
    ...
    // LITE DIFF: Full-3D engines removed.  gru_triangle_engine is replaced by
    // the Lite triangle shell; gru_depth_rw is gone (depth opcodes are rejected
    // at dispatch, see depth_q_wr_en / cmd_cfg_error above).  The shell drains
    // the triangle command FIFO in retire order and emits a done pulse (so the
    // Fence retires) plus a cfg_error pulse (no triangle opcode is implemented
    // in Phase 0x0).  It never emits spans.
    gru_lite_triangle_engine u_gru_lite_triangle_engine (
        ...
        .cmd_done_pulse (lite_tri_cmd_done_pulse),
        .cfg_error_pulse(lite_tri_cfg_error_pulse)
    );

    // LITE DIFF: depth engine removed.  Depth commands never reach the FIFO, so
    // tie the depth-engine outputs that fence/status/retire still read.
    assign depth_cmd_ready        = 1'b1;   // fence_complete ANDs depth_cmd_ready
    assign depth_cmd_done_pulse   = 1'b0;   // no depth command ever retires
    ...
    // LITE DIFF: blit slave-side responses into the native 128-bit blit engine.
    // The depth/texture read masters no longer exist, so the gating reduces to
    // cmd_fetch vs blit on the read channel (and blit vs writer on the write
    // channel); see the simplified mux below.
    assign blit_s_arready = m_axi_arready & sim_ar_allow & ~use_cmd_fetch_read;
    assign blit_s_rid     = m_axi_rid;
    assign blit_s_rdata   = m_axi_rdata;
    assign blit_s_rresp   = m_axi_rresp;
    assign blit_s_rlast   = m_axi_rlast;
    assign blit_s_rvalid  = m_axi_rvalid & sim_r_allow & ~use_cmd_fetch_read;
    ...
    // LITE DIFF: only two read masters (cmd_fetch, blit) and two write masters
    // (blit, writer).  depth/texture branches removed (no such engines).
    assign use_cmd_fetch_read = (cmd_fetch_axi_arvalid | cmd_fetch_axi_rready);
    assign use_blit_write = blit_bus_awvalid | blit_bus_wvalid | blit_bus_bready;
    assign m_axi_arid    = use_cmd_fetch_read ? cmd_fetch_axi_arid    : blit_bus_arid;
    assign m_axi_araddr  = use_cmd_fetch_read ? cmd_fetch_axi_araddr  : blit_bus_araddr;
    assign m_axi_arlen   = use_cmd_fetch_read ? cmd_fetch_axi_arlen   : blit_bus_arlen;
    assign m_axi_arsize  = use_cmd_fetch_read ? cmd_fetch_axi_arsize  : blit_bus_arsize;
    assign m_axi_arburst = use_cmd_fetch_read ? cmd_fetch_axi_arburst : blit_bus_arburst;
    assign m_axi_arlock  = use_cmd_fetch_read ? cmd_fetch_axi_arlock  : blit_bus_arlock;
    assign m_axi_arcache = use_cmd_fetch_read ? cmd_fetch_axi_arcache : blit_bus_arcache;
    assign m_axi_arprot  = use_cmd_fetch_read ? cmd_fetch_axi_arprot  : blit_bus_arprot;
    assign m_axi_arvalid = (use_cmd_fetch_read ? cmd_fetch_axi_arvalid : blit_bus_arvalid) & sim_ar_allow;
    assign m_axi_rready  = use_cmd_fetch_read ? (cmd_fetch_axi_rready & sim_r_allow) : (blit_bus_rready & sim_r_allow);
    assign m_axi_awid    = use_blit_write ? blit_bus_awid    : writer_axi_awid;
    assign m_axi_awaddr  = use_blit_write ? blit_bus_awaddr  : writer_axi_awaddr;
    assign m_axi_awlen   = use_blit_write ? blit_bus_awlen   : writer_axi_awlen;
    assign m_axi_awsize  = use_blit_write ? blit_bus_awsize  : writer_axi_awsize;
    assign m_axi_awburst = use_blit_write ? blit_bus_awburst : writer_axi_awburst;
    assign m_axi_awlock  = use_blit_write ? blit_bus_awlock  : writer_axi_awlock;
    assign m_axi_awcache = use_blit_write ? blit_bus_awcache : writer_axi_awcache;
    assign m_axi_awprot  = use_blit_write ? blit_bus_awprot  : writer_axi_awprot;
    assign m_axi_awvalid = (use_blit_write ? blit_bus_awvalid : writer_axi_awvalid) & sim_aw_allow;
    assign m_axi_wdata   = use_blit_write ? blit_bus_wdata   : writer_axi_wdata;
    assign m_axi_wstrb   = use_blit_write ? blit_bus_wstrb   : writer_axi_wstrb;
    assign m_axi_wlast   = use_blit_write ? blit_bus_wlast   : writer_axi_wlast;
    assign m_axi_wvalid  = (use_blit_write ? blit_bus_wvalid : writer_axi_wvalid) & sim_w_allow;
    assign m_axi_bready  = use_blit_write ? (blit_bus_bready & sim_b_allow) : writer_axi_bready;
    ...
    gru_axi_writer #(
        ...
        .m_axi_awready (m_axi_awready & sim_aw_allow & ~use_blit_write),
        .m_axi_wdata   (writer_axi_wdata),
        .m_axi_wstrb   (writer_axi_wstrb),
        .m_axi_wlast   (writer_axi_wlast),
        .m_axi_wvalid  (writer_axi_wvalid),
        .m_axi_wready  (m_axi_wready & sim_w_allow & ~use_blit_write),
        .m_axi_bid     (m_axi_bid),
        .m_axi_bresp   (m_axi_bresp),
        .m_axi_bvalid  (m_axi_bvalid & ~use_blit_write),
    ...
    always_ff @(posedge aclk or negedge aresetn) begin
        if (!aresetn) begin
            ...
            // LITE DIFF: Lite triangle shell flags CFG_ERROR for every drained
            // triangle (no opcode implemented yet).
            if (lite_tri_cfg_error_pulse) begin
                status_cfg_error <= 1'b1;
            end
            ...
            // LITE DIFF: Lite triangle shell contributes its own done pulse so
            // drained triangles retire in order alongside the 2D/blit/depth paths.
            if (writer_cmd_done_pulse || blit_cmd_done_pulse || depth_cmd_done_pulse || lite_tri_cmd_done_pulse) begin
