    ...
    if (reset) begin
        ...
        main_lookup: begin
            if (req_or_inst_valid && main_lookup2lookup) begin
                ...
                // DEBUG: Print cache eviction info
                if (SIMULATION && !uncache_wr && (replace_d && replace_v)) begin
                    //$display("[DCACHE_EVICT] time=%0t, idx=0x%02x, tag=0x%05x, will_writeback, way=%0b, replace_d=%b, replace_v=%b",
                     //        $time, index, tag, replace_way, replace_d, replace_v);  // commented out
                end
            end
            else begin
                main_state <= main_idle;
...
// DEBUG: Print cache line writeback data
generate if(SIMULATION) begin
    always @(posedge clk) begin
        if (main_state_is_miss && wr_rdy) begin
            if (~uncache_wr_buffer) begin
                //$display("[DCACHE_WRITEBACK] time=%0t, wr_addr=0x%08x, replace_data=%032h, way0=%032h, way1=%032h",
	                         //$time, {replace_tag, request_buffer_index, 4'b0}, replace_data,
	                         //way_data[0], way_data[1]);
            end
        end
    end
end
...
// DEBUG: Print CPU write operations
generate if(SIMULATION) begin
    always @(posedge clk) begin
        if (main_state_is_lookup && cache_hit && request_buffer_op && !cancel_req) begin
            //$display("[DCACHE_CPU_WRITE] time=%0t, addr=0x%08x, tag=0x%05x, idx=0x%02x, offset=0x%x, wdata=0x%08x, wstrb=0x%x, way_hit=%0b",
	                     //$time, {request_buffer_tag, request_buffer_index, request_buffer_offset},
	                     //request_buffer_tag, request_buffer_index, request_buffer_offset,
	                     //request_buffer_wdata, request_buffer_wstrb, way_hit);
        end
        if (main_state_is_refill && ret_valid && request_buffer_op &&
            (request_buffer_offset[3:2] == miss_buffer_ret_num)) begin
            //$display("[DCACHE_REFILL_MERGE] time=%0t, line_addr=0x%08x, word_sel=%0d, ret_data=0x%08x, req_wdata=0x%08x, req_wstrb=0x%x, write_in=0x%08x, refill_data=0x%08x",
	                     //$time, {request_buffer_tag, request_buffer_index, 4'b0},
	                     //miss_buffer_ret_num, ret_data, request_buffer_wdata,
	                     //request_buffer_wstrb, write_in, refill_data);
        end
    end
end
endgenerate
