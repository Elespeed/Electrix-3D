...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Query VCD facts with bounded JSON output.")
...
def build_signal_catalog(references: list[str]) -> tuple[dict[str, list[int]], set[str]]:
...
def resolve_signal(query: str, references: list[str]) -> dict[str, object]:
    ...
    groups, scalars = build_signal_catalog(references)
...
def allowed_profile_prefixes(args: argparse.Namespace, vcd_path: Path) -> list[str]:
    if not args.phase and not args.profile:
        return []
    ...
    tb = vcd_path.parent.name
    selected_profiles = resolve_profiles(manifest, tb, args.phase, args.profile)
    patterns = profile_patterns(manifest, tb, selected_profiles)
    prefixes = []
    for pattern in patterns:
        normalized = normalize_signal_name(pattern).rstrip("*").rstrip(".")
        if normalized:
...
def main() -> None:
    ...
    references = list(vcd.references_to_ids.keys())
    profile_prefixes = allowed_profile_prefixes(args, vcd_path)
    if profile_prefixes:
        references = [
            ref for ref in references
            if any(ref == prefix or ref.startswith(prefix + ".") or ref.startswith(prefix + "[") for prefix in profile_prefixes)
        ]
    ...
    if args.action == "value.at":
        ...
        if resolved["kind"] == "scalar":
            value = value_for_tick(vcd, resolved["name"], tick)
        else:
            value = bus_value_for_tick(vcd, resolved["name"], resolved["bits"], tick)
        payload = {
            "ok": True,
            "action": args.action,
    ...
    if resolved["kind"] == "scalar":
        tv = [(int(at), value) for at, value in vcd[resolved["name"]].tv if start_tick <= int(at) <= end_tick]
        initial = value_for_tick(vcd, resolved["name"], start_tick)
    else:
        tv = []
        initial = bus_value_for_tick(vcd, resolved["name"], resolved["bits"], start_tick)
        seen_ticks = sorted({
            int(at)
            for bit in resolved["bits"]
            for at, _ in vcd[f"{resolved['name']}[{bit}]"].tv
            if start_tick <= int(at) <= end_tick
        })
        for at in seen_ticks:
            value = bus_value_for_tick(vcd, resolved["name"], resolved["bits"], at)
            tv.append((at, value))
    ...
    if args.action == "edge.find":
        if resolved["kind"] != "scalar":
            ...
            sys.exit(1)
        prev = value_for_tick(vcd, resolved["name"], start_tick)
        matches = []
        for at, value in tv:
