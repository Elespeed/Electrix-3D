...
def invoke(root:Path,model:str,run_id:str,tb:str)->None:
    run=root/"experiments/3d_scene/runs"/run_id/model; raw=run/"raw"; out=run/"validated"; raw.mkdir(parents=True,exist_ok=False); out.mkdir()
    config=root/"experiments/3d_scene/configs/formal.json"; asset=root/"experiments/3d_scene/assets"/model/"model.s3d.bin"
    manifest={"schema":"scene-controller-run/v1","run_id":run_id,"model":model,"asset_sha256":sha(asset),"config_sha256":sha(config),"raw":"raw/transcript.log","frame_dump_limit":0}
    ...
    subprocess.run(["wsl", "bash", "-lc", f"cd '{linux_dir}' && make BENCH_MODE=SCENE_CONTROLLER RT3D_SIMULATION=1 RT3D_MODEL={model}"], cwd=root, check=True)
    # The TB parameter is compile-time; keep MIF selection explicit instead of
    # overwriting the shared default asset.
    command=["make","-C",str(root/"fpga/verilator"),f"TB={tb}","run-batch","CLEAN_FRAME_OUTPUT=0","RUN_ARGS=+UART_ECHO",f"VERILATOR_EXTRA=-DRT3D_MODEL_{model}"]
...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument("--root",type=Path,required=True); ap.add_argument("--model"); ap.add_argument("--models",nargs="+"); ap.add_argument("--run-id",required=True); ap.add_argument("--tb",default="rt_3d_soc_tb"); a=ap.parse_args()
    for model in a.models or [a.model]: invoke(a.root,model,a.run_id,a.tb)
    return 0
