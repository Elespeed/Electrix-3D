...
# The asynchronous SRAM bus is driven directly from the AXI-to-SRAM adapter.
# Default SLOW output buffers consumed ~3.6 ns of the already constrained
# clock-to-out budget; use FAST slew on the complete, board-local SRAM bus so
# address/data/control timing remains matched across BaseRAM and ExtRAM.
set_property SLEW FAST [get_ports {
    base_ram_addr[*] base_ram_data[*] base_ram_be_n[*]
    base_ram_ce_n base_ram_oe_n base_ram_we_n
    ext_ram_addr[*] ext_ram_data[*] ext_ram_be_n[*]
    ext_ram_ce_n ext_ram_oe_n ext_ram_we_n
}]

# The Scene controller is an AXI read-only master.  Its MODEL_BASE register
# changes only while the CPU writes the Scene MMIO window; that transaction is
# terminated by the Scene slave and cannot assert an ExtRAM write.  The shared
# AXI/SRAM datapath nevertheless contains this structural, unsensitizable arc.
# Use the exact startpoint and endpoint reported by Vivado: a sequential C pin
# is a valid -from object, and an inout top-level port is a valid -to object.
# Do not broaden this exception to other SRAM ports or MODEL_BASE bits.
set_false_path \
    -from [get_pins {g_scene_ctrl/u_scene_ctrl/u_regs/model_base_reg[3]/C}] \
    -to [get_ports {ext_ram_data[24]}]
