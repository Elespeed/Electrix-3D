...
# Match the software framebuffer/GRU/GDU register configuration to the RTL
# profile selected by `make CONFIG_PROFILE=<name>`. Remote is the default for
# the BRAM-backed 400x300 -> 800x600 SketchBook system.
CONFIG_PROFILE ?= remote
ifeq ($(CONFIG_PROFILE),remote)
CFLAGS += -DSOC_CONFIG_PROFILE_REMOTE
else ifeq ($(CONFIG_PROFILE),blade2x2)
CFLAGS += -DSOC_CONFIG_PROFILE_BLADE2X2
else ifeq ($(CONFIG_PROFILE),blade2x2sim)
CFLAGS += -DSOC_CONFIG_PROFILE_BLADE2X2SIM
else ifeq ($(CONFIG_PROFILE),complex)
CFLAGS += -DSOC_CONFIG_PROFILE_COMPLEX
else ifeq ($(CONFIG_PROFILE),sim)
CFLAGS += -DSOC_CONFIG_PROFILE_SIM
else ifeq ($(CONFIG_PROFILE),sim320x240)
CFLAGS += -DSOC_CONFIG_PROFILE_SIM320X240
else
...
C_SRCS   += $(COMMON_DIR)/drivers/matmul.c
C_SRCS   += $(COMMON_DIR)/drivers/uart_io.c
C_SRCS   += $(COMMON_DIR)/drivers/led.c \
			$(COMMON_DIR)/drivers/seg7.c \
			$(COMMON_DIR)/drivers/fb_cache.c \
			$(COMMON_DIR)/drivers/gdu_fb.c \
			$(COMMON_DIR)/drivers/lcd_refresh.c \
			$(COMMON_DIR)/drivers/gfx_swap.c \
			$(COMMON_DIR)/drivers/gru.c \
			$(COMMON_DIR)/drivers/gru_cmd_buffer.c \
			$(COMMON_DIR)/drivers/gru_3d.c \
			$(COMMON_DIR)/drivers/gfx_font.c \
			$(COMMON_DIR)/drivers/gfx2d.c \
			$(COMMON_DIR)/drivers/gfx_perf.c \
			$(COMMON_DIR)/drivers/sketchbook.c
